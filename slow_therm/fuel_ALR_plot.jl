...
L = 14
T = 50
data = deserialize("data/slow_therm/fuel_ALR/L$(L)_T$(T).dat")
...
errors = Float64[]
L_list = [10:17; 19;]
T_list = [5,10,30,50,100,300,500,1000,5000]
...
plot!(L_list, 2^(linear_fit.coeffs[1]) * 2 .^ (linear_fit.coeffs[2]*L_list),
      lw=2,
      c=:blue,
      label=L"0.108 \cdot 2^{1.333 L}",
      legend=:bottomright,
      )
xlabel!(L"L")
ylabel!(L"t_\mathrm{cr}")
# title!(L"L=%$(L), T=%$(int2label(T))", titlefontsize=20)
savefig("pics/slow_therm/fuel_ALR/averaged/tcr(L).pdf")
